...
func addBook(bookName string, quantityValue int) error {
	...
	fmt.Printf("The book %v added successfully with  quantity %v  and the book has not been Issued\n", bookName, quantity[bookName])
	return nil
...
func updateBookName(oldbookName, newbookName string) error {
	// update book name
	if _, ok := quantity[oldbookName]; !ok {
		return errors.New("Book does not exist!")
	}

	quantity[newbookName] = quantity[oldbookName]

	delete(quantity, oldbookName)
...
func issueBook(bookName string, quantityValue int) error {
	...
	if quantity[bookName] != neverIssued || quantity[bookName] != returned {
		quantity[bookName] = borrowed
		fmt.Printf("The bookName %v is borrowed\n", bookName)
	}
...
func returnBook(bookName string) error {
	...
	if quantity[bookName] == borrowed {
		quantity[bookName] = returned
...
func display(bookName string) error {
	fmt.Printf("Book name %v of status %v", bookName, quantity[bookName])
	return nil
}

func displayAllBook() error {

	for title, details := range quantity {
